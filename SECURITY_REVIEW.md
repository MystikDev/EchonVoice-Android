...
- Release 2.0.24 was published with owner authorization through the GitHub release workflow on September 6, 2026. The APK downloaded through the live website link matches `distribution/latest.json`, has the expected package/version and previous release signing identity, and passes signature, 16 KiB packaging alignment, and GitHub provenance verification. `security-review/artifacts.json` records the distributed artifact separately from the locally tested APKs. The CI artifact has not received additional device testing.
